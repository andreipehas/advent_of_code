...
def traverse(lines: List[str], x: int, y: int, start_direction: Direction) -> int:
    explore_queue = [(x, y, start_direction)]
        ...
        with redirect_stdout(f):
            with open(file_path, 'r') as file:
                ...
                for i in range(len(lines)):
                    sol = max(sol, traverse(lines, i, 0, Direction.EAST))
                    sol = max(sol, traverse(lines, i, len(lines[0]) - 1, Direction.WEST))
                
                for i in range(len(lines[0])):
                    sol = max(sol, traverse(lines, 0, i, Direction.SOUTH))
                    sol = max(sol, traverse(lines, len(lines) - 1, i, Direction.NORTH))
