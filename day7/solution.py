...
def is_five_of_a_kind(tally: List[Tuple[int, int]]) -> bool:
   return tally[0][1] == 5

def is_four_of_a_kind(tally: List[Tuple[int, int]]) -> bool:
   return tally[0][1] == 4

def is_full_house(tally: List[Tuple[int, int]]) -> bool:
   return tally[0][1] == 3 and tally[1][1] == 2

def is_three_of_a_kind(tally: List[Tuple[int, int]]) -> bool:
   return tally[0][1] == 3

def is_two_pair(tally: List[Tuple[int, int]]) -> bool:
   return tally[0][1] == 2 and tally[1][1] == 2

def is_one_pair(tally: List[Tuple[int, int]]) -> bool:
   return tally[0][1] == 2

def determine_type(hand: str) -> int:
    tally = Counter(hand).most_common(2)

    if is_five_of_a_kind(tally):
       return 6
    
    if is_four_of_a_kind(tally):
       return 5
    
    # Full house?
    if is_full_house(tally):
       return 4
    
    # Three of a kind?
    if is_three_of_a_kind(tally):
       return 3
    
    # Two pair?
    if is_two_pair(tally):
       return 2
    
    # One pair?
    if is_one_pair(tally):
       return 1
